 ...
 *   5. simplify with Douglas-Peucker and normalise into a unit box
 *
...
rowBands.forEach((rb, ri) => {
  ...
  cells.forEach((cb) => {
    ...
    // Normalise into a unit box, preserving aspect.
...
fs.writeFileSync(
  ...
  `'use strict';\n` +
    ...
    `   Traced from ${path.basename(file)}. Coordinates are normalised to a\n` +
    `   unit box centred on the origin; fill even-odd. */\n\n` +
    `const GLYPH_PATHS = [\n${body}\n];\n`,
  'utf8'
);
